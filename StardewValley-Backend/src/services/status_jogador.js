...
export async function obterStatusJogador(idJogador) {
  // console.log(`Connection string: ${process.env.CONNECTION_STRING}`);
  const client = await connect();

  const query = `SELECT j.nome, j.energia, j.qtdd_ouro, h.nivel_coleta, h.nivel_cultivo, h.nivel_mineracao, h.nivel_pesca, h.nivel_pesca, h.nivel_combate FROM jogador j 
